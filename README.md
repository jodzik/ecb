...
| 4 | `data_id` | 2 | Идентификатор данных |
| 6 | `payload` | 0..4096 | Данные, зависящие от типа пакета |
| 6 + N | `checksum` | 4 | CRC32/ISO-HDLC, Little Endian |
...
Одновременно на шине только одна активная транзакция.

Мастер не может начинать новую транзакцию, пока слейв не ответ на текущую или она не завершится ошибкой таймаута.  

Слейв также имеет свои таймауты на подготовку ответов на те или иные endpoints,
причем на стороне слейва они должны быть чуть меньше чем на мастере.  
Т.е. слейв не должен отвечать на запрос после истечения таймаута.
...
Если в момент подготовки ответа на запрос слейву приходят очередные байты кадра/пакета,
то слейв должен дождаться полного приема пакета, и отправлять ответ только в том случае,
если это был retry на текущую транзакцию, иначе текущую транзакцию считать завершенной с ошибкой.

Слейв не знает о том, успешно ли дошел его ответ мастеру, это достоверно знает только мастер.
Это необходимо учитывать при проектировании модели взаимодействия.

Например: слейв имеет счетчик в отдельной RAM, который необходимо обнулять при чтении,
тогда может произойти такая последовательность:
- мастер отправляет запрос на чтение;
- слейв формирует ответ и обнуляет счетчик;
- у слейва происходит аппаратная неполадка последовательного интерфейса;
- мастер принимает решение о перезагрузке слейва и данные теряются так и не дойдя до мастера.

Поэтому в данной ситуации лучше использовать другой паттерн: слейв не обнуляет счетчик при чтении,
а вместо этого имеет endpoint вычитания счетчика:
- мастер отправляет запрос на чтение;
- слейв формирует ответ;
- у слейва происходит аппаратная неполадка последовательного интерфейса;
- мастер принимает решение о перезагрузке слейва, но данные все еще в выделенной RAM.
- после перезагрузки успешно читает счетчик и записывает полученное значение в endpoint вычитания.

В случае отсутствия возможности/необходимости сохранения данных в независимую RAM протокол уже
представляет максимальные гарантии доставки.
